...
function createFallbackSvg(board = {}, adTitles = []) {
  ...
  const rows = [
    adTitles[0] || '현장별 모집공고를 한 곳에서 확인',
    adTitles[1] || '광고 자세히 보기 · 전화문의 · 문자문의',
  ].map(escapeXml)
...
<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630" viewBox="0 0 1200 630">
  ...
  <text x="112" y="418" font-family="system-ui, sans-serif" font-size="30" font-weight="900" fill="#071321">1. ${rows[0]}</text>
  <text x="112" y="488" font-family="system-ui, sans-serif" font-size="30" font-weight="900" fill="#071321">2. ${rows[1]}</text>
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!dataUrl || typeof dataUrl !== 'string' || !/^data:image\/jpeg;base64,/.test(dataUrl) || dataUrl.length < 20000) {
      const adIds = Array.isArray(board.adIds) ? board.adIds.slice(0, 3) : []
      const ads = await Promise.all(adIds.map(async (adId) => ID_PATTERN.test(String(adId)) ? redis.get(`ad:${adId}`) : null))
      const adTitles = ads
        .filter(Boolean)
        .map((ad) => String(ad.cardTitle || ad.title || '').trim())
        .filter(Boolean)
      const image = await sharp(Buffer.from(createFallbackSvg(board, adTitles), 'utf8')).jpeg({ quality: 92 }).toBuffer()
